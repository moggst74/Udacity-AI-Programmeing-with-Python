...
def main():
    ...
    # Classify Prediction
    top_probs, top_labels, top_flowers = predict(in_arg.image_path, model, in_arg.category_names, in_arg.top_k, in_arg.gpu)
...
def predict(image_path, model, category_names, topk, cpu_gpu):
    ...
    # use GPU if it is availalbe
    use_cuda = torch.cuda.is_available() and cpu_gpu == 'gpu'
    
    if use_cuda:
        model.cuda() # use GPU if it is available
    else:
        model.cpu() # # use CPU if GPU not available
    
    processed_image = process_image(image_path)
    processed_image = processed_image.unsqueeze_(0).float()
    model.eval()
    
    if use_cuda:
        with torch.no_grad():
            probs = torch.exp(model.forward(processed_image.cuda()))# convert processed_image to cuda
    else:
        with torch.no_grad():
            probs = torch.exp(model.forward(processed_image))# don't convert processed_image to cuda

    top_probs, top_labs = probs.topk(topk)

    with open(category_names, 'r') as f: # load in a mapping from category label to category name
    ...
    np_top_labs = top_labs[0].cpu().numpy()  # copy tensor to cpu and convert to numpy array
    ...
    if use_cuda:
        top_probs = top_probs[0].detach().cpu().numpy() #converts from tensor to nparray
    else:
